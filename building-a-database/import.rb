...
zoning_types = csv_records.map {|record| record["zoning_type"]}
zoning_types.uniq!

  zoning_types.each do |type|
    db_connection do |conn|
      result = conn.exec_params('SELECT name FROM zoning_types WHERE name=$1',
      [type])
...
construction_types.uniq!
  ...
  construction_types.each do |type|
    db_connection do |conn|
      result = conn.exec_params('SELECT name FROM construction_types WHERE name=$1',[type])
